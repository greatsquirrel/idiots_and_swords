...
TRAUMAS = {
    'Сломанное ребро':  {'desc': '-1 к вашим\nдействиям',       'color': (80, 50, 50)},
    'Грусть':           {'desc': 'Сбросьте две\nверхние карты',  'color': (50, 50, 80)},
    'Головокружение':   {'desc': '-1 усилие\nмозгов',            'color': (60, 60, 50)},
    'Обида':            {'desc': '+1 к действиям\nсоперника',    'color': (70, 50, 60)},
    'Исчерпание':       {'desc': 'Замешайте 2\nкарты усталости\nв колоду', 'color': (90, 85, 80)},
    'Пьянство':         {'desc': '-1 действие\nна перерыве',     'color': (70, 60, 40)},
}
...
def _draw_colored(draw, text, x, y, font, base=(255, 255, 255)):
    kw, pos = _find_first_kw(text)
    if kw is None:
        draw.text((x, y), text, fill=base, font=font)
        return
    if pos > 0:
        draw.text((x, y), text[:pos], fill=base, font=font)
    ...
    x += _text_w(kw, font)
    if pos + len(kw) < len(text):
        _draw_colored(draw, text[pos + len(kw):], x, y, font, base)
...
def _draw_text_block(draw, text, x, y, w, font, base=(40, 40, 40)):
    ...
    for i, line in enumerate(lines):
        if _has_colored(line):
            ...
            lx = x + (w - cw) // 2
            _draw_colored(draw, line, lx, y + i * line_h, font, base)
        else:
            tw = _text_w(line, font)
            lx = x + (w - tw) // 2
            draw.text((lx, y + i * line_h), line, fill=base, font=font)
...
def _render_half(draw, name, text, x0, y0, w, h, is_top, default_type='Атака'):
    ...
    font_effect = _get_font(14)

    tw = _text_w(name, font_name)
    nx = x0 + (w - tw) // 2
    ny = sy + (stripe_h - 18) // 2
    _draw_outlined(draw, name, nx, ny, font_name, (255,255,255), stripe_color, 2)
    ...
    if effect:
        pad = 14
        _draw_text_block(draw, effect, x0 + pad, by + 10, w - pad * 2, font_effect, (40, 40, 40))
...
def create_single_card(name, text):
    ...
    if effect:
        pad = 20
        _draw_text_block(draw, effect, pad, 88, CARD_W - pad * 2, font_effect, (40, 40, 40))
